...
public class IMPCubeLut: IMPImage {
    
    
    /// CubeLut export generator version
    public static let version = "1.0"
    
    public var generatorComment = "# Generated by IMPCubeLut v." + IMPCubeLut.version
    ...
    /// Cube type
    public var type:CubeType { return _type }
    
    /// Cube title
    public var title:String { return _title }
    
    /// Current cube precision presentation
    public var format:CubeFormat {return _format }
    
    fileprivate var _format:CubeFormat = .float
    fileprivate var _type:CubeType = .lut_3d
...
// MARK: - Create linear Cube LUT
public extension IMPCubeLut {
    
    /// Create liner Cube Lut
    ///
    /// - Parameters:
    ///   - context: context
    ///   - lutSize: lut size
    ///   - lutType: cube type: CubeType.lut_3d or .lut_1d
    ///   - format: cube number format: CubeFormat.integer or .float
    ///   - title: cube file title
    public convenience init(context: IMPContext, lutSize:Int, lutType:CubeType = .lut_3d, format:CubeFormat = .integer, title:String? = nil) throws {
        self.init(context: context, storageMode:nil)
        _title    = title ?? "IMPCubeLut \(lutSize):\(lutType):\(format)"
        _lutSize  = lutSize
        _type     = lutType
        _format = format
        ...
        guard let text = texture else { throw CubeError(file: "", line: 0, kind: .empty) }
        
        let kernel = IMPFunction(context: context, kernelName: _type == .lut_1d ? "kernel_make1DLut" : "kernel_make3DLut")
    ...
    fileprivate func makeTexture() -> MTLTexture {
        ...
        textureDescriptor.textureType = _type == .lut_1d ? .type1D : .type3D
...
public extension IMPCubeLut {
    ...
    public func export(to path: String) throws {
        try autoreleasepool{
            guard let txt = texture else { throw CubeError(file: path, line: 0, kind: .empty) }
            
            let file:UnsafeMutablePointer<FILE>! = fopen((path as NSString).utf8String, "w")
            
            guard file != nil else {  throw CubeError(file: path, line: 0, kind: .notCreated) }
...
extension IMPCubeLut {
    fileprivate func update(cube path: String) throws {
        
        if (FileManager.default.fileExists(atPath: path) == false) {
            throw CubeError(file: path, line: 0, kind: .notFound)
        }
        
        let file:UnsafeMutablePointer<FILE>! = fopen((path as NSString).utf8String, "r")
        
        guard file != nil else {  throw CubeError(file: path, line: 0, kind: .notFound) }
        ...
        try autoreleasepool{
            ...
            while let line = self.readline(file) {
                ...
                if words.count <= 1 {
                    throw CubeError(file: path, line: linenumber, kind: .wrangFormat)
                }
                ...
                if keyword.hasPrefix("TITLE") {
                    ...
                    if  _lutSize < 2 || _lutSize > 256  {
                        throw  CubeError(file: path, line: linenumber, kind: .outOfRange)
                    }
                }
                    ...
                    if _lutSize == 0 {
                        throw CubeError(file: path, line: linenumber, kind: .empty)
                    }
